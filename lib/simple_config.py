...
class SimpleConfig(PrintError):
    ...
    def get_wallet_path(self):
        ...
        old_path = os.path.join(self.path, "electrum-arg.dat")
...
def read_system_config(path=SYSTEM_CONFIG_PATH):
    ...
    if os.path.exists(path):
        try:
            ...
            print "cannot parse electrum-arg.conf. please install ConfigParser"
            return
...
def read_user_config(path):
    """Parse and store the user config settings in electrum-arg.conf into user_config[]."""
    if not path:
        return {}
